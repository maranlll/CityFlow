...
bool RoadNet::loadFromJson(std::string jsonFileName) { // 从 json 读入
    ...
    try {
        ...
        //  read intersections
        std::map<std::string, RoadLinkType> typeMap = {{"turn_left", turn_left}, {"turn_right", turn_right}, {"go_straight", go_straight}};
        ...
        for (rapidjson::SizeType i = 0; i < interValues.Size(); i++) {
                ...
                for (const auto &laneLinkValue : laneLinksValue.GetArray()) {
                    ...
                    if (iter != laneLinkValue.MemberEnd() && !iter->value.Empty())
                        for (const auto &pValue : iter->value.GetArray()) {
                            laneLink.points.emplace_back(getJsonMember<double>("x", pValue), getJsonMember<double>("y", pValue));
                        }
                    else {
                        Point start = Point(startLane->getPointByDistance(startLane->getLength() - startLane->getEndIntersection()->width));
                        Point end = Point(endLane->getPointByDistance(0.0 + endLane->getStartIntersection()->width));
                        double len = (Point(end.x - start.x, end.y - start.y)).len();
                        Point startDirection = startLane->getDirectionByDistance(startLane->getLength() - startLane->getEndIntersection()->width);
                        Point endDirection = endLane->getDirectionByDistance(0.0 + endLane->getStartIntersection()->width);
                        double minGap = 5;
            ...
            for (const auto &lightPhaseValue : lightPhasesValue.GetArray()) {
                ...
                const auto &availableRoadLinksValue = getJsonMemberArray("availableRoadLinks", lightPhaseValue);
...
std::vector<Point> Intersection::getOutline() { // ?
    ...
    auto minIter = std::min_element(points.begin(), points.end(), [](const Point &a, const Point &b) { return a.y < b.y; });
    ...
    std::sort(points.begin(), points.end(), [&p0](const Point &a, const Point &b) { return (a - p0).ang() < (b - p0).ang(); });
