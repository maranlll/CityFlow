...
double Vehicle::getNoCollisionSpeed(double vL, double dL, double vF, double dF, double gap, double interval, double targetGap) const {
...
double Vehicle::getCarFollowSpeed(double interval) {
    ...
    // collision free
    double v =
        getNoCollisionSpeed(leader->getSpeed(), leader->getMaxNegAcc(), vehicleInfo.speed, vehicleInfo.maxNegAcc, controllerInfo.gap, interval, 0);
...
int Vehicle::getReachSteps(double distance, double targetSpeed, double acc) const {
    ...
    if (distanceUntilTargetSpeed > distance) {
        return std::ceil((std::sqrt(vehicleInfo.speed * vehicleInfo.speed + 2 * acc * distance) - vehicleInfo.speed) / acc / interval);
    } else {
...
double Vehicle::getIntersectionRelatedSpeed(double interval) {
    ...
    if (nextDrivable && nextDrivable->isLaneLink()) {
        ...
        if (!laneLink->isAvailable() || !laneLink->getEndLane()->canEnter(this)) { // not only the first vehicle should follow intersection logic
            if (getMinBrakeDistance() > controllerInfo.drivable->getLength() - controllerInfo.dis) {
                // TODO: what if it cannot brake before red light?
            } else {
                v = min2double(v, getStopBeforeSpeed(controllerInfo.drivable->getLength() - controllerInfo.dis, interval));
                return v;
            }
        }
        if (laneLink->isTurn()) {
    ...
    double distanceToLaneLinkStart =
        controllerInfo.drivable->isLane() ? -(controllerInfo.drivable->getLength() - controllerInfo.dis) : controllerInfo.dis;
