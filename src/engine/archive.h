...
class Archive {
  private:
    ...
    template <typename T>
    static void addObjectAsMember(rapidjson::Value &jsonObject, const std::string &name, const T &object,
                                  rapidjson::MemoryPoolAllocator<> &allocator) {
        if (object) {
            jsonObject.AddMember(rapidjson::Value(name, allocator).Move(), rapidjson::Value(object->getId(), allocator).Move(), allocator);
        }
    }

    template <typename T>
    static void pushBackObjectAsMember(rapidjson::Value &jsonObject, const T &object, rapidjson::MemoryPoolAllocator<> &allocator) {
        if (object) {
            jsonObject.PushBack(rapidjson::Value(object->getId(), allocator).Move(), allocator);
        }
    }
