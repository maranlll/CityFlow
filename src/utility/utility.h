...
class JsonMemberMiss : public JsonFormatError {
  ...
  public:
    explicit JsonMemberMiss(const std::string &name) : JsonFormatError(name + " is required but missing in json file") {}
};
...
class JsonTypeError : public JsonFormatError {
  ...
  public:
    JsonTypeError(const std::string &name, const char *type) : JsonFormatError(name + ": expected type " + type) {}

    JsonTypeError(const std::string &name, const std::string &type) : JsonTypeError(name, type.c_str()) {}
};
...
template <typename T> T getJsonMember(const std::string &name, const rapidjson::Value &object) {
...
template <typename T> T getJsonMember(const std::string &name, const rapidjson::Value &object, const T &default_value) {
    assert(object.IsObject());
