...
struct Option {
    ...
    static std::string get_destination(std::string opt1, std::string opt2, OptionType ft, OptionType st);
};
//----------------------------------------------------------------------------
...
std::string Option::get_destination(std::string first_option, std::string second_option, OptionType first_opt_type, OptionType second_opt_type) {
    std::string dest;
...
class OptionParser {
  public:
    OptionParser(std::string description = "", bool create_help = true) : m_options(0), m_description(description) {
        if (create_help) {
  ...
  private:
    ...
    bool get_value_arg(std::vector<std::string> &arguments, unsigned int &arg, Option &opt, std::string &flag);

    bool try_to_get_opt(std::vector<std::string> &arguments, unsigned int &arg, Option &option, std::string &flag);

    void check_for_missing_args();
};

Option &OptionParser::add_option(std::string first_option, std::string second_option) {
    return add_option_internal(first_option, second_option);
}

Option &OptionParser::add_option_internal(std::string first_option, std::string second_option) {
...
bool OptionParser::get_value_arg(std::vector<std::string> &arguments, unsigned int &arg, Option &opt, std::string &flag) {
    std::string val = "";
    ...
    if (arguments[arg].size() > flag.size()) {
        ...
        if (search_pt == std::string::npos) {
            ...
            if (search_pt == std::string::npos) {
                error("Error, long options (" + flag + ") require a '=' or space before a value.");
                return false;
            }
...
bool OptionParser::try_to_get_opt(std::vector<std::string> &arguments, unsigned int &arg, Option &option, std::string &flag) {
    if (flag == "") {
        return false;
    }
    ...
    if (((option.mode() == STORE_VALUE) || (option.mode() == STORE_MULT_VALUES)) && (option.found() == false)) {
...
//----------------------------------------------------------------------------
template <> unsigned int OptionParser::get_value<unsigned int>(std::string key) {
    try {
...
//----------------------------------------------------------------------------
template <> std::vector<std::string> OptionParser::get_value<std::vector<std::string>>(std::string key) {
    try {
        return m_values[key];
    } catch (std::out_of_range &err) {
        std::string e("Tried to access value for field '");
...
//----------------------------------------------------------------------------
template <> std::vector<int> OptionParser::get_value<std::vector<int>>(std::string key) {
    try {
        std::vector<int> v;
