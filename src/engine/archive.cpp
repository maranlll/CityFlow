...
Archive::Archive(const Engine &engine)
    : step(engine.step), activeVehicleCount(engine.activeVehicleCount), rnd(engine.rnd), finishedVehicleCnt(engine.finishedVehicleCnt),
...
rapidjson::Value Archive::dumpVehicle(const Vehicle &vehicle, rapidjson::Document &jsonRoot) const {
    ...
    vehicleValue.AddMember("id", rapidjson::Value(vehicle.getId(), allocator).Move(), allocator);
    ...
    vehicleValue.AddMember("approachingIntersectionDistance", vehicle.controllerInfo.approachingIntersectionDistance, allocator);
    vehicleValue.AddMember("gap", vehicle.controllerInfo.gap, allocator);
    vehicleValue.AddMember("enterLaneLinkTime", static_cast<unsigned>(vehicle.controllerInfo.enterLaneLinkTime), allocator);
...
void Archive::dumpDrivables(rapidjson::Document &jsonRoot) const {
    ...
    for (const auto &iter : drivablesArchive) {
        ...
        drivablesValue.AddMember(rapidjson::Value(drivable->getId(), allocator).Move(), drivableValue, allocator);
    }
...
void Archive::dumpFlows(rapidjson::Document &jsonRoot) const {
    ...
    for (const auto &iter : flowsArchive) {
        ...
        flowsValue.AddMember(rapidjson::Value(flow->getId(), allocator).Move(), flowValue, allocator);
    }
...
void Archive::dumpTrafficLights(rapidjson::Document &jsonRoot) const {
    ...
    for (const auto &iter : trafficLightsArchive) {
        ...
        trafficLightsValue.AddMember(rapidjson::Value(intersection->getId(), allocator).Move(), trafficLightValue, allocator);
    }
...
Archive::Archive(Engine &engine, const std::string &filename) {
    ...
    for (auto &vehicleValue : vehiclesValue.GetArray()) {
        ...
        Vehicle *vehicle = new Vehicle(vehicleInfo, getJsonMember<const char *>("id", vehicleValue), &engine);
        ...
        controllerInfo.approachingIntersectionDistance = getJsonMember<double>("approachingIntersectionDistance", vehicleValue);
